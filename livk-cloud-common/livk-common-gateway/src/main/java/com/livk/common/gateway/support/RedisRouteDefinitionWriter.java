...
public class RedisRouteDefinitionWriter implements RouteDefinitionRepository {
	...
	/**
	 * 从Redis拿下来是一个LinkedHashMap 使用反射直接操作，uri类型为{@link java.net.URI}
	 * 使用JackSon进行序列化和反序列化，将Map转成{@link com.livk.common.gateway.domain.LivkRoute}
	 * @return Flux<LivkRoute>
	 */
	public Flux<RouteDefinition> getRouteDefinitions() {
		return Flux.fromIterable(livkRedisTemplate.opsForHash().entries(ROUTE_KEY).values().stream()
				.map(JacksonUtil::toJson).map(str -> JacksonUtil.toBean(str, LivkRoute.class)).toList());
	}
