...
public interface DynamicRouteConverter extends BaseConverter<RedisRoute, DynamicRoute> {
    ...
    @Mapping(target = "predicates", expression = "java(JacksonUtil.toStream(dynamicRoute.getPredicates(),Predicate.class).toList())")
    @Mapping(target = "filters", expression = "java(JacksonUtil.toStream(dynamicRoute.getFilters(),Filter.class).toList())")
    @Mapping(target = "metadata", expression = "java(JacksonUtil.toMap(dynamicRoute.getMetadata(),String.class,Object.class))")
    @Override
